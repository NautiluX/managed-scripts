...
type CliOpts struct {
	BaseImage string `short:"b" long:"baseimg" default:"quay.io/your-user/managed-scripts" description:"Base Image" required:"false"`
}
...
func main() {
	...
	job := createJob(metadata, opts, targetScript)
...
func createJob(metadata Metadata, options CliOpts, script string) v1.Pod {
	envs := []v1.EnvVar{}
...
func (m *Metadata) GetPodCommand(script string) []string {
	filePath := fmt.Sprintf("/managed-scripts/" + script + "/" + m.File)
	if m.Language == "bash" {
